...
网页预览会按 `PLAN.md` 的简谱规则渲染：`^` / `,` 分别显示为数字上方/下方加点，`_` / `=` 显示为一条/两条下划线，`.` 显示为附点，`0` 显示为休止符，`-` 可作为后缀或独立 token 显示为延音，`()` 显示为连音弧且只占一个歌词字位。歌词对齐时，连续英文字母（如 `oh`、`Hoo`）按一个字位处理；后端填词会跳过这些英文位并在输出中保留原词。
